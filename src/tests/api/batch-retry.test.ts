...
const { POST } = await import('@/app/api/batches/[batchId]/retry/route');
...
const SAMPLE_BATCH = {
  ...
  status_message: 'Previous failure',
  file_count: 1,
  voucher_count: 0,
...
const PIPELINE_OUTPUT = {
  ...
  voucherCount: 1,
  ledgerCount: 1,
  checks: [{ check_name: 'Voucher Balance', status: 'PASSED' as const, details: 'OK' }],
...
describe('POST /api/batches/[batchId]/retry', () => {
  ...
  it('returns 404 when batch does not exist', async () => {
    ...
    expect((await res.json()).error).toContain('Batch not found');
  });
  ...
  it('returns 409 when batch is in uploading state (not retryable)', async () => {
    repo.getBatch.mockResolvedValueOnce({ ...SAMPLE_BATCH, status: 'uploading' });
  ...
  it('returns 409 when batch is in succeeded state (not retryable)', async () => {
    repo.getBatch.mockResolvedValueOnce({ ...SAMPLE_BATCH, status: 'succeeded' });
  ...
  it('accepts needs_review status as retryable', async () => {
    repo.getBatch.mockResolvedValueOnce({ ...SAMPLE_BATCH, status: 'needs_review' });
  ...
  it('returns 409 when batch has no files', async () => {
  ...
  it('returns 409 when files are not in uploaded state', async () => {
    repo.getFilesByBatch.mockResolvedValueOnce([
      { ...SAMPLE_FILES[0], status: 'failed', file_name: 'tradebook.csv' },
    ]);
  ...
  it('returns 200 with expected response shape on success', async () => {
    const { request, params } = makeRequest();
    const res = await POST(request, { params });
    const body = await res.json();
  ...
  it('sets batch status to running before calling the pipeline', async () => {
    const { request, params } = makeRequest();
  ...
  it('returns 500 and marks batch failed when pipeline throws', async () => {
    mockRunProcessingPipeline.mockRejectedValueOnce(new Error('Pipeline exploded'));
    ...
    expect(repo.updateBatchStatus).toHaveBeenCalledWith('batch-1', 'failed', 'Pipeline exploded');
  });

  it('returns 500 when storage path is missing for a file', async () => {
